...
def main():
    ...
    SRC_PATH = r'E:\Speaker-Diarization-master\ghostvlad\dataset'
    path_spk_tuples = prepare_data(SRC_PATH)
    print(len(path_spk_tuples))
    train_sequence = []
    train_cluster_id = []

    for epoch in range(20): # Random choice utterances from whole wavfiles
        # A merged utterance contains [10,20] utterances
        splits_count = np.random.randint(1, 13, 1)
        print(splits_count)
